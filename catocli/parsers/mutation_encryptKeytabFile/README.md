
## CATO-CLI - mutation.encryptKeytabFile:
[Click here](https://api.catonetworks.com/documentation/#mutation-mutation.encryptKeytabFile) for documentation on this operation.

### Usage for mutation.encryptKeytabFile:

```bash
catocli mutation encryptKeytabFile -h

catocli mutation encryptKeytabFile <json>

catocli mutation encryptKeytabFile --json-file mutation.encryptKeytabFile.json

catocli mutation encryptKeytabFile '{"accountId":"id","encryptKeytabFileInput":{"file":"example_value"}}'

catocli mutation encryptKeytabFile '{
    "accountId": "id",
    "encryptKeytabFileInput": {
        "file": "example_value"
    }
}'
```

#### Operation Arguments for mutation.encryptKeytabFile ####

`accountId` [ID] - (required) N/A    
`encryptKeytabFileInput` [EncryptKeytabFileInput] - (required) N/A    
