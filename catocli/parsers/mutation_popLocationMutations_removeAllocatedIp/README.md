
## CATO-CLI - mutation.popLocationMutations.removeAllocatedIp:
[Click here](https://api.catonetworks.com/documentation/#mutation-mutation.popLocationMutations.removeAllocatedIp) for documentation on this operation.

### Usage for mutation.popLocationMutations.removeAllocatedIp:

```bash
catocli mutation popLocationMutations removeAllocatedIp -h

catocli mutation popLocationMutations removeAllocatedIp <json>

catocli mutation popLocationMutations removeAllocatedIp --json-file mutation.popLocationMutations.removeAllocatedIp.json

catocli mutation popLocationMutations removeAllocatedIp '{"accountId":"id","popLocationRemoveAllocatedIpInput":{"ip":"example_value","type":"SYSTEM"}}'

catocli mutation popLocationMutations removeAllocatedIp '{
    "accountId": "id",
    "popLocationRemoveAllocatedIpInput": {
        "ip": "example_value",
        "type": "SYSTEM"
    }
}'
```

#### Operation Arguments for mutation.popLocationMutations.removeAllocatedIp ####

`accountId` [ID] - (required) N/A    
`popLocationRemoveAllocatedIpInput` [PopLocationRemoveAllocatedIpInput] - (required) N/A    
