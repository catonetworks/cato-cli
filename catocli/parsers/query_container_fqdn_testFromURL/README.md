
## CATO-CLI - query.container.fqdn.testFromURL:
[Click here](https://api.catonetworks.com/documentation/#query-query.container.fqdn.testFromURL) for documentation on this operation.

### Usage for query.container.fqdn.testFromURL:

```bash
catocli query container fqdn testFromURL -h

catocli query container fqdn testFromURL <json>

catocli query container fqdn testFromURL --json-file query.container.fqdn.testFromURL.json

catocli query container fqdn testFromURL '{"testContainerFromUrlInput":{"fileType":"STIX","url":"example_value"}}'

catocli query container fqdn testFromURL '{
    "testContainerFromUrlInput": {
        "fileType": "STIX",
        "url": "example_value"
    }
}'
```

#### Operation Arguments for query.container.fqdn.testFromURL ####

