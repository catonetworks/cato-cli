
## CATO-CLI - query.policy.ztnaAlwaysOn.policy:
[Click here](https://api.catonetworks.com/documentation/#query-query.policy.ztnaAlwaysOn.policy) for documentation on this operation.

### Usage for query.policy.ztnaAlwaysOn.policy:

```bash
catocli query policy ztnaAlwaysOn policy -h

catocli query policy ztnaAlwaysOn policy <json>

catocli query policy ztnaAlwaysOn policy --json-file query.policy.ztnaAlwaysOn.policy.json

catocli query policy ztnaAlwaysOn policy '{"ztnaAlwaysOnPolicyInput":{"revision":{"id":"id","type":"PRIVATE"}}}'

catocli query policy ztnaAlwaysOn policy '{
    "ztnaAlwaysOnPolicyInput": {
        "revision": {
            "id": "id",
            "type": "PRIVATE"
        }
    }
}'
```

#### Operation Arguments for query.policy.ztnaAlwaysOn.policy ####

`accountId` [ID] - (required) N/A    
`ztnaAlwaysOnPolicyInput` [ZtnaAlwaysOnPolicyInput] - (required) N/A    
